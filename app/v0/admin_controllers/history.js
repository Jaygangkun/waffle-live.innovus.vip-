        ...
        if (response.success) {
            ...
            if (search_value !== ""){
                ...
                {    
                    ...
                    query1['store_detail.name'] = {$regex: new RegExp(search_value, 'i')};
                    var search = {"$match": {$or: [query1]}};
            ...
            var project = {$project: {delivery_type: "$delivery_type",
                                            unique_id: "$unique_id",
                                            provider_detail: "$provider_detail",
            ...
            if (page) {
                        ...
                        Order.aggregate([filter, condition1, created_by_condition, order_type_condition, order_status_id_condition,request_query,array_to_json_request_query,pickup_type_condition, payment_status_condition, sort,project, search, skip, limit]).then((orders) => {
                            if (orders.length === 0) {
...
exports.delivery_list_search_sort = function (request_data, response_data) {
    utils.check_request_params(request_data.body, [], function (response) {
        if (response.success) {
            ...
            var project = {$project: {created_at: "$created_at",
                                            timezone: "$timezone",
                                            user_detail: "$user_detail",
                                            provider_detail: "$provider_detail",
                                            is_paid_from_wallet: "$is_paid_from_wallet",
            ...
            if(page) {
                ...
                limit["$limit"] = number_of_rec;
                        ...
                        Request.aggregate([delivery_status_condition, request_status_condition, delivery_status_manage_id_condition, orders_array_unwind, order_query, array_to_json_order_query, order_condition,  payment_status_condition,project, sort, search, skip, limit]).then((requests) => {
                ...
                Request.aggregate([delivery_status_condition, request_status_condition, delivery_status_manage_id_condition, orders_array_unwind, order_query, array_to_json_order_query, order_condition, pickup_type_condition, payment_status_condition, sort, search]).then((requests) => {
                    if (requests.length === 0) {
                        response_data.json({success: false, error_code: ORDER_ERROR_CODE.ORDER_NOT_FOUND});
                    } else
                    {
