        ...
        if (response.success) {
            ...
            if (search_value !== ""){
                ...
                } else if (search_field == 'unique_id')
                {
                    ...
                    {
                        search_value = Number(search_value);
                        query1[search_field] = search_value;
            ...
            var project = {$project: {is_schedule_order: "$is_schedule_order",
                    ...
                    if (page)
                    {
                        Order.aggregate([order_condition, created_by_condition, order_type_condition,  request_status_condition,payment_status_condition, pickup_type_condition,project, sort, search, skip, limit
                        ]).then((orders) => {
                            if (orders.length == 0) {
                                response_data.json({success: false, error_code: ORDER_ERROR_CODE.ORDER_NOT_FOUND});
                            } else
                        ...
                        Order.aggregate([order_condition, created_by_condition, order_type_condition, request_status_condition,payment_status_condition, pickup_type_condition, sort, search
                        ]).then((orders) => {
                            if (orders.length == 0) {
                                response_data.json({success: false, error_code: ORDER_ERROR_CODE.ORDER_NOT_FOUND});
                            } else {
