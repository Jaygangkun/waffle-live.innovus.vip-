                        ...
                        Order.aggregate([user_condition, order_status_condition, filter,
                            ...
                            { "$unwind": "$order_payment_detail" },
                            {
                                $project: {
                                    ...
                                    store_detail: { name: { $ifNull: [{$arrayElemAt: [ "$store_detail.name", Number(request_data.headers.lang) ]}, { $ifNull: [{$arrayElemAt: [ "$store_detail.name", 0 ]}, ""] }] }, image_url: { $cond: ["$store_detail", "$store_detail.image_url", ''] } }
                                }
                            },
                        ]).then((orders) => {
