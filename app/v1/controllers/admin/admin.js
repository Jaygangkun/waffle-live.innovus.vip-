    ...
    utils.check_request_params(request_data.body, [], function (response) {
        ...
        if (response.success) {
            ...
            Table.findOne({referral_code: request_data_body.referral_code}).then((detail) => {
                if (detail)
                {
					//console.log(detail.country_id);
					//console.log(request_data_body.country_id);
                    // details.forEach(function (detail) {
                    if (detail.country_id == request_data_body.country_id)
                    {
                        var query = { $or: [{ 'country': detail.country_id }, { 'country_code': detail.country_code }] };
                        Country.findOne(query, function (error, country) {
                            ...
                            switch (type) {
                                case ADMIN_DATA_ID.USER:
									...
									/* if(country.no_of_user_use_referral !=== null ){
										no_of_use_referral = country.no_of_user_use_referral;
									}else{
										no_of_use_referral = 1;
									} */
									
									no_of_use_referral = 10;
