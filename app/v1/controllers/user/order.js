                            ...
                            if (order) {
                                Order_payment.findOne({_id: order.order_payment_id, store_id: request_data_body.store_id}).then((order_payment) => {
                                    ...
                                    if(order_status == ORDER_STATE.STORE_ACCEPTED){
                                        console.log(order.order_change);
                                        if(order.order_change == false){
                                            exports.capture_payment(store, order, request_data, order_payment, function(capture_payment_response){
                                                if(capture_payment_response.success){
                                                    exports.store_accept_order(store, order, request_data, order_payment, function(store_accept_response){
                                            ...
                                            response_data.json({
                                                success: false,
                                                error_code: ORDER_ERROR_CODE.ORDER_NOT_ACCEPTED
                                            });
                                        }
...
exports.capture_payment = function(store, order, request_data, order_payment, response_data){
    if(order_payment.is_payment_mode_cash == false && order_payment.capture_amount >0){
        ...
        Payment_gateway.findOne({_id: order_payment.payment_id}).then((payment_gateway) => {
						...
						request({
							uri: 'https://pi-test.sagepay.com/api/v1/transactions/'+order_payment.payment_intent_id+'/instructions/',
							auth: {
								user: 'HDDQDyHKef95qwUdJHE5EkLpf9G5uet9VVZjQGv2jqne9Eh2py',
								password: 'Nx9rI7fNs7jpFuPpCXj8UboZ0AtlwnNbihLEMS22xWNMJBmJYoItJcecPpgM9Gi2W'
							},
							headers: {
								"Authorization": 'Basic SEREUUR5SEtlZjk1cXdVZEpIRTVFa0xwZjlHNXVldDlWVlpqUUd2MmpxbmU5RWgycHk6Tng5ckk3Zk5zN2pwRnVQcENYajhVYm9aMEF0bHduTmJpaExFTVMyMnhXTk1KQm1KWW9JdEpjZWNQcGdNOUdpMlc=',
								'Content-Type': 'application/json',
							},
							method: "POST",
							json: true,
							body: {
								"instructionType": "release","amount":Math.round((order_payment.card_payment * 100)),
							}

						}, function (error, response, body) {
							console.log("capture_payment");
							console.log(body);
							if(body.instructionType == "release"){
								if(order_payment.wallet_payment > 0){
									var wallet_amount = order_payment.wallet_payment;
									...
									User.findOne({_id: order_payment.user_id}).then((user) => {
											...
											var total_wallet_amount = wallet_history.add_wallet_history(ADMIN_DATA_ID.USER, user.unique_id, user._id, user.country_id
																, user.wallet_currency_code, order_payment.order_currency_code, order_payment.wallet_to_order_current_rate, wallet_amount, user.wallet,
																WALLET_STATUS_ID.REMOVE_WALLET_AMOUNT, WALLET_COMMENT_ID.ORDER_CHARGED, "Order Charged: "+order.unique_id , wallet_information );
											user.wallet = total_wallet_amount;
											user.save();
									...
									//order_payment.total_after_wallet_payment = (order_payment.cash_payment + order_payment.card_payment) - order_payment.wallet_payment;
								}else{
									response_data({
											success: true
										});
							...
							//response_data.json({success: true, merchantSessionKey: body.merchantSessionKey})
						})
...
exports.capture_payment_old = function(store, order, request_data, order_payment, response_data){
    if(order_payment.is_payment_mode_cash == false && order_payment.capture_amount >0){
        ...
        Payment_gateway.findOne({_id: order_payment.payment_id}).then((payment_gateway) => {
            ...
            stripe.paymentIntents.capture(
              order_payment.payment_intent_id,{
                  amount_to_capture: order_payment.card_payment * 100,
                ...
                if(!err){
                    ...
                    /*if((order_payment.capture_amount - order_payment.remaining_payment) > 0){
                        ...
                        User.findOne({_id: order_payment.user_id}).then((user) => {
                            ...
                            var total_wallet_amount = wallet_history.add_wallet_history(ADMIN_DATA_ID.USER, user.unique_id, user._id, user.country_id
                                                , user.wallet_currency_code, order_payment.order_currency_code, order_payment.wallet_to_order_current_rate, wallet_amount, user.wallet,
                                                  WALLET_STATUS_ID.ADD_WALLET_AMOUNT, WALLET_COMMENT_ID.ORDER_REFUND, "Refund Order Charged" , wallet_information );
                            user.wallet = total_wallet_amount;
                            user.save();
                        })
                        //order_payment.total_after_wallet_payment = (order_payment.cash_payment + order_payment.card_payment) - order_payment.wallet_payment;
                    }*/
                    //order_payment.card_payment =  order_payment.capture_amount;
                    order_payment.remaining_payment =0;
                    order_payment.save(function(err){
                        response_data({
                            success: true
...
exports.store_accept_order = function(store, order, request_data, order_payment, response_data){
                ...
                if (order_payment) {
                    ...
                    order.save().then((detail) => {
                        ...
                        User.findOne({_id: order.user_id}).then((user) => {
                            ...
                            if (added_wallet > 0 && order_status != ORDER_STATE.WAITING_FOR_ACCEPT_STORE) {
								...
								utils.refund_payment_for_selected_payment_gateway(order_payment, function (response) {
                ...
                if (order_payment) {
                    ...
                    order.save().then(() => {
                        ...
                        User.findOne({_id: order.user_id}).then((user) => {
                            ...
                            if (order_wallet_payment > 0 && order_status != ORDER_STATE.WAITING_FOR_ACCEPT_STORE) {
    ...
    utils.check_request_params(request_data.body, [{name: 'order_id', type: 'string'}], function (response) {
        if (response.success) {
            ...
            User.findOne({_id: user_id}).then((user) => {
                if (user)
                {
                    if (request_data_body.type !== ADMIN_DATA_ID.ADMIN && (request_data_body.server_token !== null && user.server_token !== request_data_body.server_token))
                    {
                        response_data.json({success: false, error_code: ERROR_CODE.INVALID_SERVER_TOKEN});
                    } else
                    {
                        var query = { $or: [{ '_id': user.country_id }, { 'country_code': user.country_code }] };
...
exports.approve_edit_order = function (request_data, response_data) {
                        ...
                        Order.findOne({_id: request_data_body.order_id, user_id: user_id}).then((order) => {
                            if (order) {
                                ...
                                order.save(function(err){
                                    response_data.json({
                                        success: true,
                                        message: ORDER_MESSAGE_CODE.ORDER_APPROVE_SUCCESSFULLY
                                    });
                                    Store.findOne({_id:order.store_id}).then((store) => {
                                        if(store){
                                            utils.sendPushNotification(ADMIN_DATA_ID.STORE, store.device_type, store.device_token, STORE_PUSH_CODE.USER_ACCEPT_EDIT_ORDER, PUSH_NOTIFICATION_SOUND_FILE.PUSH_NOTIFICATION_SOUND_FILE_IN_IOS);
                                        }
                                    });
                                });
                            } else
                                        ...
                                        Request.findOne({_id: order.request_id}).then((request) => {
                                            Provider.findOne({_id: request.provider_id}).then((provider) => {
                                                ...
                                                if(store){
                                                    ...
                                                    store_data.name = String(store.name[Number(request_data.headers.lang)]);
