				 ...
				 if (request_data_body.server_token !== null && detail.server_token !== request_data_body.server_token)
                    ...
                    } else
                    {
                        ...
                        Payment_gateway.findOne({_id: request_data.body.payment_id}).then((payment_gateway) => {
							var card = null;
							if(request_data.body.is_save == 0){
								card = {
									"cardIdentifier":request_data.body.payment_method,
									"reusable": true
								}
							}else if(request_data.body.is_save == 1){
								card = {
							...
							if(type == 2){
								detail.first_name = detail.name[0];
								detail.last_name = detail.name[0];
							}
							...
							request({
								...
								body: {
									...
									"vendorTxCode":detail._id+'-'+utils.generateServerToken(4),
									"amount":Math.round((request_data_body.wallet * 100)),
									"currency":"GBP",
									"customerFirstName":detail.first_name,
									"customerLastName":detail.last_name,
									"billingAddress":{
										"address1":detail.first_name,
										"address2":detail.address,
										"city":request_data_body.city,
										"postalCode":request_data_body.postcode,
										"country":"GB",
										"state":null
									},
								...
								if(body.statusCode == "0000"){
									if(request_data.body.is_save == 1){
										...
										var wallet = Number((body.amount.totalAmount / 100).toFixed(2));     
										var wallet_currency_code = detail.wallet_currency_code;
											var total_wallet_amount = wallet_history.add_wallet_history(type, detail.unique_id, detail._id, detail.country_id, wallet_currency_code, wallet_currency_code,
                                                    1, wallet, detail.wallet, WALLET_STATUS_ID.ADD_WALLET_AMOUNT, WALLET_COMMENT_ID.ADDED_BY_CARD, "Card : " + body.paymentMethod.card.lastFourDigits)
