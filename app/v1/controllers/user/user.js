...
var card_stripe = require('../../controllers/user/card');
                                ...
                                if (user_detail) {
                                    ...
                                    User.findOne({ phone: request_data_body.phone }).then((user_detail) => {
                                        if (user_detail) {
                                            ...
                                            var referral_code = utils.generateReferralCode(ADMIN_DATA_ID.ADMIN, request_data_body.currency, first_name, last_name);
                                            ...
                                            if (city.length == 0) {
                                                ...
                                                if (city[0].is_business) {
                                                    ...
                                                    if (!request_data_body.is_courier) {
                                                        var ads = [];
                                                        var condition = {"$match": {$and: [{"_id": {$in: city[0].deliveries_in_city}}, {is_business: {$eq: true}}]}};
                                                        ...
                                                        var project = {
                                                           ...
                                                           var array_to_json_famous_products_tags_detail = {
                                                                    $unwind: {
                                                             ...
                                                             $group:
                                                              {
                                                                 ...
                                                                 famous_products_tags: {$addToSet: { $ifNull: [{$arrayElemAt: [ "$famous_products_tags", Number(request_data.headers.lang) ]}, { $ifNull: [{$arrayElemAt: [ "$famous_products_tags", 0 ]}, ""] }] }},
                                                                 unique_id: { $first: '$unique_id' }
                                                              }
                                                           }
                                                        var sort = { "$sort": {} };
                                                        sort["$sort"]['sequence_number'] = parseInt(1);
                                                        Delivery.aggregate([condition,sort,project,array_to_json_famous_products_tags_detail,group], function (error, delivery) {
                                                            console.log(error);
                                                            if (delivery.length == 0) {
                                                                ...
                                                                var project = {
                                                                    $project: {
                                                                        ...
                                                                        image_for_mobile: 1,
                                                                        expiry_date: 1,
                                                                        "store_detail":{$cond:{ if: {$ifNull: ["$store_detail",false]},then:{
...
exports.get_store_list = function (request_data, response_data) {
    ...
    utils.check_request_params(request_data.body, [{ name: 'city_id', type: 'string' }, { name: 'store_delivery_id', type: 'string' }], function (response) {
        if (response.success) {
            ...
            var ads_condition = {"$match": {$and: [{"ads_for": {$eq: ADS_TYPE.STORE_LIST}}, {is_ads_visible: {$eq: true}}, {is_ads_approve_by_admin: {$eq: true}}]}};
    ...
    utils.check_request_params(request_data.body, [{ name: 'store_id', type: 'string' }], function (response) {
        if (response.success) {
            ...
            if(request_data_body.product_ids){
                ...
                var condition2 = { "$match": { '_id': { $in: product_ids } } }; 
            }else{
                                ...
                                Product.aggregate([condition2,condition, condition1,
                                    ...
                                    products.forEach(function (product) {
                                        if (product && product._id &&
                                            (product._id.product_time == undefined || product._id.product_time == null || product._id.product_time.length != 7)) {
                                            
                                            product._id.product_time = [ 
                                                {
                                    ...
                                    var store_detail = JSON.parse(JSON.stringify(store))
                                    ...
                                    var delivery_detail = JSON.parse(JSON.stringify(delivery_data))
...
exports.user_get_specification_list = function (request_data, response_data) {
        var request_data_body = request_data.body;
                    ...
                    Item.findOne({_id: request_data_body.item_id}).then((item_detail) => {
                        if(item_detail){
                            var specification_group = [];
                            
                            item_detail.specifications.forEach(function(spec_group,index1){
                                var spec_group_name = spec_group.name[Number(request_data.headers.lang)];
                                ...
                                spec_group.list.forEach(function(spec,index){
                                    var spec_name = spec.name[Number(request_data.headers.lang)];
                                    ...
                                    if (finalCityId != null) {
                                        ...
                                        City.aggregate([cityid_condition, delivery_query,unwind,city_project]).then((city) => {
                                            if (city.length == 0) {
                                                ...
                                                if (city[0].is_business) {
                                                    var ads = [];
                                                    var condition = {"$match": {$and: [{"city_id": {$eq: city[0]._id}}, {store_delivery_id: {$eq: mongoose.Types.ObjectId(request_data_body.store_delivery_id)}}, {is_approved: {$eq: true}}, {is_business: {$eq: true}}]}};
                                                    ...
                                                    var sort = { "$sort": {} };
                                                    sort["$sort"]['sequence_number'] = parseInt(1);
                                                    Store.aggregate([condition,project,sort]).then((stores) => {
                                                        if (stores.length == 0) {
                                                            response_data.json({
                                                                success: false,
            ...
            User.findOne({ _id: request_data_body.user_id }).then((user_detail) => {
                if (user_detail) {
                    ...
                    if (request_data_body.server_token !== null && user_detail.server_token !== request_data_body.server_token) {
                        ...
                        Order.aggregate([user_condition, order_payment_query,  order_invoice_condition, array_to_json_order_payment_query, order_status_condition,
                                    ...
                                    Country.findOne({ _id: country_id }).then((country) => {
                                        ...
                                        City.findOne({ _id: city_id }).then((city_detail) => {
                                            if (city_detail) {
                                                ...
                                                var radius = false;
                                                var distance = utils.getDistanceFromTwoLocation(city_detail.city_lat_long, cart.destination_addresses[0].location);
                                                            ...
                                                            utils.check_zone(city_id, delivery_type, delivery_price_used_type_id, service.vehicle_id, city_detail.zone_business, store_location, destination_location, function (zone_response) {
                                                                ...
                                                                if (service) {
                                                                    //if (service.admin_profit_mode_on_delivery) {
                                                                        var admin_profit_mode_on_delivery = service.admin_profit_mode_on_delivery;
                                                                        var admin_profit_value_on_delivery = service.admin_profit_value_on_delivery;
                                                                    //}
                                                                ...
                                                                if(setting_detail.tip_type == 1){
                                                                    var tip_amount = (request_data_body.tip_amount * total_order_price)/100;
                                                                    tip_amount = utils.precisionRoundTwo(Number(tip_amount));
                                                                }else{
                                                                    var tip_amount = request_data_body.tip_amount;
                                                                }
                                                                total_provider_income =total_provider_income + +tip_amount;
                                                                var total = +total_delivery_price + +total_order_price + +tip_amount;
                                                                    ...
                                                                    if (!is_payment_mode_cash) {
                                                                        ...
                                                                        if (order_payment.remaining_payment > 0) {
																			card_stripe.get_stripe_payment_intent({
                                                                                ...
                                                                                is_save:request_data.body.is_save,
                                                                                merchantSessionKey:request_data.body.merchantSessionKey,
                                                                                last4:request_data.body.last_four,
                                                                                card_holder_name:request_data.body.card_holder_name,
                                                                                card_expiry_date:request_data.body.card_expiry_date,
                                                                                user_id:user._id,
																				delivery_address:request_data.body.delivery_address,
																				postcode:request_data.body.postcode,
                                                                                ...
                                                                                order_payment.save().then(() => {
                                                                                    if(payment_paid.success){
                                                                                        ...
                                                                                        response_data.json({
                                                                                            success: false,
																							error_message:payment_paid.error,
                                                                                            error_code: ERROR_CODE.SOMETHING_WENT_WRONG
                                                                                        });
                                                                                    }
                                            ...
                                            Provider.findOne({ _id: current_provider }).then((provider_data) => {
                                                Order_payment.findOne({ _id: order_detail.order_payment_id }).then((order_payment) => {
                                                    Cart.findOne({ _id: order_detail.cart_id }).then((cart_detail) => {
                                                        ...
                                                        if (store_data) {
                                                            var store = JSON.parse(JSON.stringify(store_data))
            ...
            User.findOne({ _id: request_data_body.user_id }).then((user_detail) => {
                if (user_detail) {
                    if (request_data_body.server_token !== null && user_detail.server_token !== request_data_body.server_token) {
                        ...
                        Order.aggregate([user_condition, order_payment_query, array_to_json_order_payment_query,  order_status_condition, filter,
            ...
            User.findOne({ _id: request_data_body.user_id }).then((user_detail) => {
                if (user_detail) {
                    if (request_data_body.server_token !== null && user_detail.server_token !== request_data_body.server_token) {
                        ...
                        Order.aggregate([order_condition, order_payment_query, cart_query, request_query, store_query, array_to_json_store_detail, array_to_json_request_query, provider_query, array_to_json_cart_query, array_to_json_order_payment_query, country_query, array_to_json_country_query, payment_gateway_query,review_query
                            ...
                            if (order.length === 0) {
                                ...
                                var data = JSON.parse(JSON.stringify(order[0]));
                                if(order[0].store_detail){
                                    data.store_detail.name = String(order[0].store_detail.name[Number(request_data.headers.lang)]);
                                }
                                response_data.json({
                                    success: true,
...
// user get_order_detail
exports.get_customer_number = function (request_data, response_data) {

    utils.check_request_params(request_data.body, [{ name: 'unique_id', type: 'string' }], function (response) {
        if (response.success) {

            var request_data_body = request_data.body;
            Order.findOne({ unique_id: request_data_body.unique_id }).then((order_detail) => {
				if (order_detail) {
					response_data.json({ success: true, user_details: order_detail.user_detail });
				} else {
                    response_data.json({ success: false, error_code: USER_ERROR_CODE.ORDER_DETAIL_NOT_FOUND });
                }
            }, (error) => {
                console.log(error)
                response_data.json({
            ...
            User.findOne({ _id: request_data_body.user_id }).then((user_detail) => {
                if (user_detail) {
                    if (request_data_body.server_token !== null && user_detail.server_token !== request_data_body.server_token) {
                        ...
                        var array_to_json_delivery_detail = { $unwind: "$delivery_detail" };
                        var array_to_json_country_detail = { $unwind: "$country_detail" };
                        var server_time = new Date();
                        var project = {
                            $project: {
                        ...
                        Store.aggregate([store_id_condition, store_approve_condition, city_lookup, array_to_json_city_detail, country_lookup, array_to_json_country_detail,delivery_lookup,array_to_json_delivery_detail, project]).then((stores) => {
                                        ...
                                        if (cart_detail) {
                                            ...
                                            Order_payment.findOne({_id: order.order_payment_id}).then((order_payment) => {
                                                if (order_payment) {
                                                    ...
                                                    if(setting_detail.tip_type == 1){
                                                        var tip_amount = (order_payment.tip_value * total_order_price)/100;
                                                        tip_amount = utils.precisionRoundTwo(Number(tip_amount));
                                                    }else{
                                                        var tip_amount = order_payment.tip_value;
                                                    }
													...
													promo_code_controller.edit_order_apply_promo_code({
														...
														if(is_payment_mode_cash == false){
															if((user_pay_payment - order_payment.capture_amount) >0){
																var wallet_amount = user_pay_payment -order_payment.capture_amount;
															...
															if((user_pay_payment - order_payment.capture_amount) <=0){
																if(order_payment.capture_amount >= user_pay_payment){
																	...
																	var wallet_amount = order_payment.capture_amount -user_pay_payment;
														...
														var total_after_wallet_payment = user_pay_payment - order_payment.wallet_payment;
														total_after_wallet_payment = utils.precisionRoundTwo(total_after_wallet_payment);
														order_payment.total_after_wallet_payment = total_after_wallet_payment;
